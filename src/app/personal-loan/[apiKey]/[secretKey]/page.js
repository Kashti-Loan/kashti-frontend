import { Col, Container, Row } from "react-bootstrap";
// import { usePathname } from "next/navigation";
import styles from "./style.module.scss";
import { PageTitle, SectionTitle, Text } from "@styles/styledComponent";
import Link from "next/link";
import {
  BusinessLoanIcon,
  CreditCardBox,
  CustomImage,
  EmiCalculator,
  FaqSection,
  GoldLoanIcon,
  GreenCheckIcon,
  NormalImage,
  SecureLoanToday,
} from "@components";
import {
  citi_color,
  yesBank_color,
  unionBank_color,
  icici_color,
  personalLoanBanner,
  personalLoanBenefits,
  age,
  salaried,
  creditScore,
  selfEmployed,
  workExperience,
  instantPersonalLoan,
  flexibleTenures,
  interestRates,
  instantLoan,
  approvedLenders,
  digitalProcess,
} from "@public/assets";
import TabSection from "@components/parts/section/personalLoanPage/tabSection";
import { personalLoanTabData } from "@utils/constant";
import { routesConstant } from "@utils/routesConstant";

const Page = ({ params }) => {
  // const pathname = usePathname();
  // console.log("router", pathname);
  return (
    <main className={styles.personalLoanPage}>
      {/* Banner Section */}
      <section className={styles.bannerSection}>
        <Container>
          <Row>
            <Col xs={12} md={12} lg={7}>
              <PageTitle>Instant Loans. Trusted Providers. Check Your Eligibility!</PageTitle>
              <Text>Diverse options starting from INR 1000. Only on India's Favourite Loan Marketplace.</Text>
              <div className={styles.bannerImgBlock}>
                <div>
                  <CustomImage src={citi_color} alt='Bank Partners' />
                  <CustomImage src={unionBank_color} alt='Bank Partners' />
                  <CustomImage src={icici_color} alt='Bank Partners' />
                  <CustomImage src={yesBank_color} alt='Bank Partners' />
                  <CustomImage src={unionBank_color} alt='Bank Partners' />
                </div>
              </div>
              <div className={styles.bannerImg}>
                <CustomImage src={personalLoanBanner} />
              </div>
            </Col>
            <Col xs={12} md={12} lg={5}>
              <SecureLoanToday apiKey={params.apiKey} secretKey={params.secretKey} />
            </Col>
          </Row>
        </Container>
      </section>

      {/* Mobile - Banner Section */}
      <section className={styles.mobileBannerContainer}>
        <Container>
          <Row>
            <Col xs={7}>
              <PageTitle>Instant Loans. Trusted Providers. Check Your Eligibility!</PageTitle>
            </Col>
            <Col xs={5}>
              <NormalImage src={personalLoanBanner} alt='Personal Loan Banner' height={"116px"} />
            </Col>
          </Row>
          <Row>
            <Col>
              <SecureLoanToday />
            </Col>
          </Row>
        </Container>
      </section>

      {/* Tab Section */}
      <TabSection data={personalLoanTabData} color={"#e9f4ec"} applyLink={routesConstant.PERSONAL_LOAN_QUESTIONAIRRE} />

      {/* Key Benefits of Kashti’s Personal Loan  */}
      {/* <section className={styles.keyBenefitsSection} id="key_features">
        <Container>
          <Row>
            <Col xs={12} md={6} lg={6}>
              <SectionTitle>
                Key Benefits of Kashti’s Personal Loan{" "}
              </SectionTitle>
              <ul>
                <li>
                  <GreenCheckIcon />{" "}
                  <span>
                    <b>Quick Approval:</b> Get approved in minutes!
                  </span>
                </li>
                <li>
                  <GreenCheckIcon />{" "}
                  <span>
                    <b>Flexible Terms:</b> Tailor a repayment plan that suits
                    your budget
                  </span>
                </li>
                <li>
                  <GreenCheckIcon />{" "}
                  <span>
                    <b>Low Interest Rates:</b> Save more while meeting your
                    needs.
                  </span>
                </li>
                <li>
                  <GreenCheckIcon />{" "}
                  <span>
                    <b>No Hidden Fees:</b> Transparent and fair lending.
                  </span>
                </li>
                <li>
                  <GreenCheckIcon />{" "}
                  <span>
                    <b>Use Anywhere:</b> Whether it's a vacation, home
                    improvement, or debt consolidation, the choice is yours.
                  </span>
                </li>
              </ul>
            </Col>
            <Col xs={12} md={6} lg={6}>
              <CustomImage
                src={personalLoanBenefits}
                alt="Key Benefits of Kashti’s Personal Loan "
              />
            </Col>
          </Row>
        </Container>
      </section> */}

      <section className={styles.mobilePointerSection}>
        <Container>
          <Row>
            <Col>
              <SectionTitle>Trustworthy Loans with Kashti</SectionTitle>
            </Col>
          </Row>
          <Row>
            <Col xs={7}>
              <ul>
                <li>
                  <svg width='24' height='24' viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>
                    <g clip-path='url(#clip0_3132_19891)'>
                      <path
                        d='M11.398 5.11032L12.5597 1.5385C12.589 1.4398 12.5785 1.33354 12.5302 1.24255C12.482 1.15157 12.4 1.08314 12.3019 1.05199C10.9934 0.63054 9.62881 0.408893 8.25418 0.394531C6.87956 0.408893 5.51496 0.63054 4.20646 1.05199C4.10818 1.08298 4.026 1.15134 3.97763 1.24234C3.92927 1.33334 3.91858 1.4397 3.94788 1.5385L5.11032 5.11032V5.5033H11.398V5.11032ZM15.6894 12.5613L13.4651 21.2108C12.3318 22.2145 10.9327 22.8696 9.43606 23.0972C7.93939 23.3249 6.40882 23.1154 5.02832 22.4941C3.64782 21.8727 2.47615 20.8659 1.65412 19.5946C0.832101 18.3234 0.394712 16.8417 0.394531 15.3279C0.394531 12.2194 2.19832 8.52534 4.81558 6.68225L4.71734 5.89629H11.791L11.6928 6.68225C13.5594 7.99481 15.0095 10.2505 15.6894 12.5613Z'
                        fill='#A7ACCE'
                      />
                      <g clip-path='url(#clip1_3132_19891)'>
                        <path d='M5.89453 11.5781H10.1051' stroke='#181F51' stroke-width='0.8' stroke-linecap='round' stroke-linejoin='round' />
                        <path d='M5.89453 13.332H10.1051' stroke='#181F51' stroke-width='0.8' stroke-linecap='round' stroke-linejoin='round' />
                        <path
                          d='M5.89453 15.0898L8.87699 17.8969'
                          stroke='#181F51'
                          stroke-width='0.8'
                          stroke-linecap='round'
                          stroke-linejoin='round'
                        />
                        <path d='M5.89453 15.0898H6.94716' stroke='#181F51' stroke-width='0.8' stroke-linecap='round' stroke-linejoin='round' />
                        <path
                          d='M6.94727 15.0869C9.28656 15.0869 9.28656 11.5781 6.94727 11.5781'
                          stroke='#181F51'
                          stroke-width='0.8'
                          stroke-linecap='round'
                          stroke-linejoin='round'
                        />
                      </g>
                      <path
                        d='M11.7893 5.10938H4.71565C4.28158 5.10938 3.92969 5.46126 3.92969 5.89534C3.92969 6.32942 4.28158 6.6813 4.71565 6.6813H11.7893C12.2234 6.6813 12.5753 6.32942 12.5753 5.89534C12.5753 5.46126 12.2234 5.10938 11.7893 5.10938Z'
                        fill='#F9B47E'
                      />
                      <path
                        d='M17.6853 23.1871C20.7239 23.1871 23.1871 20.7239 23.1871 17.6853C23.1871 14.6468 20.7239 12.1836 17.6853 12.1836C14.6468 12.1836 12.1836 14.6468 12.1836 17.6853C12.1836 20.7239 14.6468 23.1871 17.6853 23.1871Z'
                        fill='#4BA264'
                      />
                      <path
                        d='M7.40339e-06 15.3263C-0.00207913 16.8833 0.436916 18.409 1.26616 19.7268C2.0954 21.0446 3.28096 22.1005 4.68555 22.7723C6.09014 23.4441 7.6563 23.7043 9.20266 23.5228C10.749 23.3412 12.2123 22.7253 13.4231 21.7465C14.4847 22.8647 15.9393 23.5265 17.4798 23.5921C19.0203 23.6577 20.5259 23.1219 21.6787 22.0979C22.8314 21.0738 23.541 19.6418 23.6574 18.1044C23.7739 16.5669 23.2881 15.0444 22.3028 13.8584C21.3174 12.6725 19.9097 11.916 18.3769 11.7487C16.8441 11.5815 15.3064 12.0167 14.0885 12.9623C12.8706 13.9079 12.068 15.2898 11.8502 16.8163C11.6324 18.3427 12.0165 19.894 12.9213 21.1425C11.8252 22.0256 10.5015 22.5805 9.10322 22.7429C7.70499 22.9053 6.28934 22.6687 5.01997 22.0603C3.75059 21.4519 2.67934 20.4967 1.93009 19.305C1.18083 18.1133 0.784169 16.7339 0.785972 15.3263C0.785972 12.2807 2.52688 8.83307 4.94019 7.07368H11.5651C12.9575 8.14468 14.6016 10.8198 15.2986 12.4323C15.3428 12.5254 15.4217 12.5974 15.5183 12.6328C15.615 12.6683 15.7218 12.6644 15.8156 12.6219C15.9094 12.5795 15.9828 12.5019 16.0201 12.4059C16.0573 12.3099 16.0553 12.2031 16.0146 12.1085C15.3369 10.5671 13.7796 7.92731 12.5102 6.821C12.6951 6.67925 12.8335 6.4855 12.9076 6.2646C12.9817 6.0437 12.9882 5.8057 12.9262 5.5811C12.8642 5.3565 12.7365 5.15554 12.5595 5.00396C12.3826 4.85239 12.1644 4.75711 11.9329 4.73033L12.9338 1.65249C12.9951 1.45504 12.9754 1.24136 12.8791 1.0584C12.7829 0.87544 12.618 0.73816 12.4206 0.676716C11.0736 0.24109 9.66824 0.0129081 8.25264 0C6.83704 0.0129081 5.43164 0.24109 4.08467 0.676716C3.98648 0.707478 3.89534 0.757333 3.81647 0.823416C3.73761 0.8895 3.67257 0.970512 3.6251 1.0618C3.57764 1.15309 3.54867 1.25285 3.53986 1.35537C3.53105 1.45788 3.54258 1.56113 3.57379 1.65917L4.57275 4.73112C4.34119 4.75793 4.12293 4.85331 3.94594 5.00501C3.76896 5.15672 3.64133 5.35783 3.57942 5.58257C3.51752 5.8073 3.52416 6.04541 3.5985 6.26634C3.67283 6.48727 3.81147 6.68096 3.99664 6.82257C2.78646 7.89311 1.80773 9.19958 1.12048 10.6619C0.433226 12.1242 0.0519412 13.7114 7.40339e-06 15.3263ZM17.6842 12.5754C18.6946 12.5754 19.6824 12.8751 20.5225 13.4364C21.3626 13.9978 22.0174 14.7957 22.4041 15.7292C22.7908 16.6627 22.892 17.6899 22.6948 18.6809C22.4977 19.6719 22.0111 20.5822 21.2967 21.2967C20.5822 22.0111 19.6719 22.4977 18.6809 22.6948C17.6899 22.8919 16.6627 22.7908 15.7292 22.4041C14.7957 22.0174 13.9978 21.3626 13.4364 20.5225C12.8751 19.6824 12.5754 18.6946 12.5754 17.6842C12.577 16.3298 13.1158 15.0312 14.0735 14.0735C15.0312 13.1157 16.3298 12.577 17.6842 12.5754ZM8.25264 0.785965C9.58601 0.805443 10.9096 1.01722 12.1825 1.41474L11.1112 4.71579H5.39448L4.32714 1.42456C5.59645 1.01706 6.91961 0.801806 8.25264 0.785965ZM4.7158 5.50175H11.7895C11.8937 5.50175 11.9937 5.54316 12.0674 5.61686C12.1411 5.69056 12.1825 5.79051 12.1825 5.89474C12.1825 5.99896 12.1411 6.09892 12.0674 6.17262C11.9937 6.24632 11.8937 6.28772 11.7895 6.28772H4.7158C4.61157 6.28772 4.51161 6.24632 4.43792 6.17262C4.36422 6.09892 4.32281 5.99896 4.32281 5.89474C4.32281 5.79051 4.36422 5.69056 4.43792 5.61686C4.51161 5.54316 4.61157 5.50175 4.7158 5.50175Z'
                        fill='#181F51'
                      />
                      <path
                        d='M16.6213 20.3209C16.695 20.3946 16.7949 20.436 16.8991 20.436C16.9159 20.4359 16.9327 20.4349 16.9494 20.4329C17.0091 20.4252 17.0662 20.4039 17.1164 20.3706C17.1665 20.3374 17.2084 20.2931 17.2387 20.2411L19.9895 15.5253C20.0179 15.4806 20.0369 15.4307 20.0454 15.3785C20.054 15.3263 20.052 15.2729 20.0394 15.2216C20.0268 15.1702 20.0041 15.1219 19.9724 15.0795C19.9407 15.0372 19.9008 15.0016 19.8551 14.9751C19.8094 14.9485 19.7587 14.9314 19.7062 14.9249C19.6538 14.9184 19.6005 14.9225 19.5496 14.937C19.4988 14.9515 19.4514 14.9762 19.4103 15.0095C19.3692 15.0428 19.3353 15.084 19.3105 15.1307L16.8166 19.4048L15.998 18.5862C15.9239 18.5147 15.8246 18.475 15.7216 18.4759C15.6186 18.4768 15.52 18.5182 15.4471 18.591C15.3743 18.6639 15.3329 18.7624 15.332 18.8655C15.3312 18.9685 15.3708 19.0678 15.4423 19.1419L16.6213 20.3209Z'
                        fill='#181F51'
                      />
                    </g>
                    <defs>
                      <clipPath id='clip0_3132_19891'>
                        <rect width='24' height='23.5789' fill='white' />
                      </clipPath>
                      <clipPath id='clip1_3132_19891'>
                        <rect width='8.42105' height='8.42105' fill='white' transform='translate(3.78906 10.5273)' />
                      </clipPath>
                    </defs>
                  </svg>

                  <span>Instant Loan Approval</span>
                </li>
                <li>
                  <svg width='24' height='25' viewBox='0 0 24 25' fill='none' xmlns='http://www.w3.org/2000/svg'>
                    <g clip-path='url(#clip0_3132_19903)'>
                      <path
                        fill-rule='evenodd'
                        clip-rule='evenodd'
                        d='M12.1582 3.54202L15.0162 6.90816C15.0698 6.97125 15.0811 7.05455 15.0463 7.12969C15.0116 7.20483 14.9407 7.25011 14.8579 7.25011H13.5268V14.1866C13.5268 14.4152 13.3403 14.6018 13.1116 14.6018H10.8882C10.6595 14.6018 10.473 14.4152 10.473 14.1866V7.25011H9.14187C9.05913 7.25011 8.98826 7.20483 8.95348 7.12969C8.9187 7.05459 8.93004 6.97125 8.98362 6.90816L11.8416 3.54202C11.8825 3.49392 11.9368 3.46875 11.9999 3.46875C12.0631 3.46875 12.1174 3.49392 12.1582 3.54202Z'
                        fill='#268F44'
                      />
                      <path
                        fill-rule='evenodd'
                        clip-rule='evenodd'
                        d='M19.0136 1.02634L21.8716 4.39248C21.9252 4.45558 21.9365 4.53892 21.9017 4.61402C21.867 4.68916 21.7961 4.73444 21.7133 4.73444H20.3822V14.1865C20.3822 14.4152 20.1957 14.6017 19.967 14.6017H17.7436C17.5149 14.6017 17.3285 14.4152 17.3285 14.1865V4.73444H15.9973C15.9145 4.73444 15.8437 4.68916 15.8089 4.61402C15.7742 4.53892 15.7855 4.45558 15.8391 4.39248L18.6971 1.02634C18.738 0.97825 18.7922 0.953125 18.8554 0.953125C18.9185 0.953125 18.9728 0.97825 19.0136 1.02634Z'
                        fill='#F57E20'
                      />
                      <path
                        fill-rule='evenodd'
                        clip-rule='evenodd'
                        d='M5.30267 6.05759L8.16068 9.42373C8.21426 9.48683 8.2256 9.57012 8.19082 9.64527C8.15609 9.72041 8.08521 9.76569 8.00248 9.76569H6.67132V14.1866C6.67132 14.4152 6.48476 14.6017 6.2561 14.6017H4.03273C3.80407 14.6017 3.61756 14.4152 3.61756 14.1866V9.76569H2.2864C2.20367 9.76569 2.13279 9.72041 2.09801 9.64527C2.06323 9.57017 2.07457 9.48687 2.12815 9.42373L4.98621 6.05759C5.02704 6.0095 5.08137 5.98438 5.14446 5.98438C5.20756 5.98442 5.26188 6.00955 5.30267 6.05759Z'
                        fill='#283487'
                      />
                      <path
                        d='M12.1617 24.2041C15.7996 24.115 18.6765 21.0937 18.5874 17.4558C18.4983 13.8179 15.4769 10.941 11.839 11.0301C8.20108 11.1192 5.32418 14.1405 5.41328 17.7784C5.50238 21.4163 8.52374 24.2932 12.1617 24.2041Z'
                        fill='#81BE93'
                      />
                      <path
                        fill-rule='evenodd'
                        clip-rule='evenodd'
                        d='M11.0817 14.1009C11.6849 14.7037 11.6869 15.6811 11.0836 16.2848C10.4808 16.888 9.50338 16.8901 8.89968 16.2868C8.30033 15.6879 8.29846 14.7025 8.89771 14.1028C9.49658 13.5035 10.4819 13.5016 11.0817 14.1009ZM15.0855 18.9397C15.6811 19.5348 15.683 20.5141 15.0875 21.11C14.4923 21.7055 13.5131 21.7075 12.9171 21.1119C12.3216 20.5168 12.3197 19.5376 12.9152 18.9416C13.5103 18.346 14.4895 18.3442 15.0855 18.9397ZM14.5859 19.4421C14.2648 19.1211 13.7389 19.1197 13.4176 19.4413C13.0966 19.7624 13.0953 20.2883 13.4168 20.6096C13.7379 20.9305 14.2638 20.9319 14.5851 20.6104C14.906 20.2892 14.9074 19.7634 14.5859 19.4421ZM10.5789 14.6064C10.2557 14.2834 9.72655 14.282 9.40321 14.6056C9.08024 14.9287 9.07883 15.4579 9.40241 15.7812C9.72655 16.1051 10.2538 16.1066 10.5781 15.782C10.9022 15.4577 10.9036 14.9309 10.5789 14.6064ZM10.0072 21.0616C9.97862 21.1016 9.94242 21.1356 9.90067 21.1616C9.85893 21.1876 9.81245 21.2051 9.76392 21.2132C9.71539 21.2212 9.66575 21.2195 9.61786 21.2083C9.56996 21.1971 9.52476 21.1765 9.48483 21.1478C9.44491 21.119 9.41105 21.0827 9.38521 21.0408C9.35937 20.999 9.34204 20.9524 9.33423 20.9039C9.32642 20.8553 9.32828 20.8057 9.3397 20.7578C9.35112 20.71 9.37187 20.6649 9.40077 20.6251L14.0701 14.1219C14.128 14.0415 14.2155 13.9874 14.3133 13.9715C14.4111 13.9555 14.5112 13.9791 14.5916 14.037C14.6314 14.0656 14.6652 14.1018 14.691 14.1436C14.7169 14.1853 14.7342 14.2317 14.7421 14.2801C14.75 14.3285 14.7483 14.378 14.737 14.4258C14.7258 14.4736 14.7052 14.5186 14.6766 14.5584L10.0072 21.0616Z'
                        fill='#181F51'
                      />
                    </g>
                    <defs>
                      <clipPath id='clip0_3132_19903'>
                        <rect width='24' height='24' fill='white' transform='translate(0 0.578125)' />
                      </clipPath>
                    </defs>
                  </svg>

                  <span>Attractive Interest Rates</span>
                </li>
                <li>
                  <svg width='24' height='25' viewBox='0 0 24 25' fill='none' xmlns='http://www.w3.org/2000/svg'>
                    <g clip-path='url(#clip0_3132_19910)'>
                      <path
                        fill-rule='evenodd'
                        clip-rule='evenodd'
                        d='M23.2751 8.46435C23.7443 9.7473 24 11.1332 24 12.5787C24 19.2058 18.6277 24.5781 12.0005 24.5781C9.21577 24.5781 6.65204 23.6289 4.61601 22.0372L3.38616 23.1077C3.27888 23.202 3.13694 23.2258 3.00366 23.1738C2.87146 23.1218 2.78369 23.0091 2.76744 22.8661L2.27875 18.4895C2.26358 18.3595 2.31234 18.2414 2.41311 18.158C2.51388 18.0745 2.63958 18.0507 2.76419 18.0897L6.81457 19.3683C6.94785 19.4095 7.04104 19.5124 7.06921 19.649C7.09739 19.7855 7.05296 19.9166 6.94785 20.0087L5.87295 20.9438C7.59041 22.2029 9.70771 22.9463 12.0005 22.9463C17.7261 22.9463 22.3692 18.3043 22.3692 12.5787C22.3692 11.4171 22.1774 10.301 21.8264 9.25861C21.7733 9.10149 21.8253 8.9357 21.9586 8.8371L22.7008 8.29098C22.7994 8.21838 22.9153 8.19888 23.0313 8.23355C23.1483 8.26931 23.2339 8.34949 23.2751 8.46435ZM12.0005 0.578125C14.7853 0.578125 17.348 1.52733 19.384 3.1191L20.6138 2.04853C20.7222 1.95534 20.8631 1.93042 20.9963 1.98243C21.1296 2.03444 21.2163 2.14713 21.2326 2.29017L21.7223 6.66671C21.7364 6.79674 21.6887 6.91484 21.5869 6.99828C21.4861 7.08171 21.3615 7.10555 21.2369 7.06654L17.1854 5.78793C17.0521 5.74676 16.96 5.64382 16.9308 5.50729C16.9026 5.37076 16.947 5.23964 17.0521 5.14862L18.127 4.2135C16.4107 2.95331 14.2923 2.20998 12.0005 2.20998C6.27387 2.20998 1.63186 6.852 1.63186 12.5787C1.63186 13.7392 1.82257 14.8552 2.17364 15.8976C2.22674 16.0548 2.17473 16.2205 2.04145 16.3192L1.2992 16.8653C1.2006 16.9379 1.08465 16.9574 0.968712 16.9227C0.851686 16.8869 0.766084 16.8068 0.724909 16.6919C0.255723 15.409 0 14.0241 0 12.5787C0 5.95047 5.37234 0.578125 12.0005 0.578125Z'
                        fill='#268F44'
                      />
                      <path
                        fill-rule='evenodd'
                        clip-rule='evenodd'
                        d='M12.0009 4.57812C16.4175 4.57812 19.9987 8.15932 19.9987 12.577C19.9987 16.9937 16.4175 20.5749 12.0009 20.5749C7.58315 20.5749 4.00195 16.9937 4.00195 12.577C4.00195 8.15932 7.58315 4.57812 12.0009 4.57812ZM12.7984 11.6029C13.0324 11.7503 13.2307 11.9486 13.3781 12.1826H16.5486C17.0926 12.1826 17.5336 12.6236 17.5336 13.1676C17.5336 13.7105 17.0926 14.1515 16.5486 14.1515H13.3781C13.0519 14.6705 12.4733 15.0162 11.8145 15.0162C10.7938 15.0162 9.96592 14.1883 9.96592 13.1676C9.96592 12.5088 10.3105 11.9301 10.8295 11.6029V8.26877C10.8295 7.72481 11.2705 7.2838 11.8145 7.2838C12.3574 7.2838 12.7984 7.72481 12.7984 8.26877V11.6029ZM11.8145 12.4839C12.1916 12.4839 12.4971 12.7905 12.4971 13.1676C12.4971 13.5447 12.1916 13.8502 11.8145 13.8502C11.4374 13.8502 11.1308 13.5447 11.1308 13.1676C11.1308 12.7905 11.4374 12.4839 11.8145 12.4839Z'
                        fill='#F57E20'
                      />
                    </g>
                    <defs>
                      <clipPath id='clip0_3132_19910'>
                        <rect width='24' height='24' fill='white' transform='translate(0 0.578125)' />
                      </clipPath>
                    </defs>
                  </svg>

                  <span>Flexible Tenures</span>
                </li>
                <li>
                  <svg width='24' height='25' viewBox='0 0 24 25' fill='none' xmlns='http://www.w3.org/2000/svg'>
                    <g clip-path='url(#clip0_3132_19914)'>
                      <path
                        d='M18.4253 15.3794C17.5441 15.3794 16.7228 15.6306 16.0216 16.0619V6.12813C16.0216 5.33687 15.3766 4.68813 14.5816 4.68813H10.5953V0.941875C10.5953 0.743125 10.4303 0.578125 10.2316 0.578125H6.77781C6.57531 0.578125 6.41406 0.743125 6.41406 0.941875V4.68813H2.41656C1.62531 4.68813 0.976562 5.33687 0.976562 6.12813V20.8469C0.976562 21.6419 1.62531 22.2869 2.41656 22.2869H14.4578C15.2566 23.6556 16.7378 24.5781 18.4253 24.5781C20.9641 24.5781 23.0266 22.5119 23.0266 19.9769C23.0266 17.4419 20.9641 15.3794 18.4253 15.3794ZM7.14156 5.05937C7.14156 5.05562 7.14156 5.05562 7.14156 5.05188C7.14156 5.04813 7.14156 5.04812 7.14156 5.04437V1.30563H9.86781V5.65938H7.14156V5.05937ZM14.1128 21.5594H2.41656C2.02281 21.5594 1.70406 21.2444 1.70406 20.8469V6.12813C1.70406 5.73438 2.02281 5.41562 2.41656 5.41562H6.41406V5.65938H5.97906C5.78031 5.65938 5.61531 5.82063 5.61531 6.02313C5.61531 6.22188 5.78031 6.38688 5.97906 6.38688H11.0266C11.2253 6.38688 11.3903 6.22188 11.3903 6.02313C11.3903 5.82063 11.2253 5.65938 11.0266 5.65938H10.5953V5.41562H14.5853C14.9791 5.41562 15.2978 5.73438 15.2978 6.12813V16.6169C14.3978 17.4569 13.8278 18.6531 13.8278 19.9806C13.8278 20.5356 13.9328 21.0644 14.1128 21.5594ZM18.4253 23.8506C16.2916 23.8506 14.5516 22.1106 14.5516 19.9769C14.5516 17.8394 16.2916 16.1031 18.4253 16.1031C20.5628 16.1031 22.2991 17.8394 22.2991 19.9769C22.2991 22.1144 20.5666 23.8506 18.4253 23.8506Z'
                        fill='#181F51'
                      />
                      <path
                        d='M18.4245 16.1055C16.2908 16.1055 14.5508 17.8417 14.5508 19.9792C14.5508 22.113 16.2908 23.853 18.4245 23.853C20.562 23.853 22.2983 22.113 22.2983 19.9792C22.2983 17.838 20.5658 16.1055 18.4245 16.1055ZM20.847 18.693L17.7683 21.7717C17.697 21.843 17.6033 21.8805 17.5095 21.8805C17.4158 21.8805 17.3258 21.843 17.2508 21.7717L16.002 20.523C15.8595 20.3805 15.8595 20.1517 16.002 20.0092C16.1445 19.8667 16.3733 19.8667 16.5158 20.0092L17.5095 20.9992L20.3333 18.1755C20.4758 18.033 20.7045 18.033 20.847 18.1755C20.9895 18.318 20.9895 18.5505 20.847 18.693Z'
                        fill='#4BA264'
                      />
                      <path
                        d='M7.14062 1.30469V5.04344C7.14062 5.04719 7.14062 5.04719 7.14062 5.05094C7.14062 5.05469 7.14062 5.05469 7.14062 5.05844V5.65844H9.86687V1.30469H7.14062ZM8.50187 3.85094C7.90562 3.85094 7.41812 3.36344 7.41812 2.76719C7.41812 2.17469 7.90562 1.68719 8.50187 1.68719C9.09437 1.68719 9.58187 2.17469 9.58187 2.76719C9.58187 3.36344 9.09437 3.85094 8.50187 3.85094Z'
                        fill='#A7ACCE'
                      />
                      <path
                        d='M15.2969 6.13047C15.2969 5.73672 14.9781 5.41797 14.5844 5.41797H10.5944V5.66172H11.0256C11.2244 5.66172 11.3894 5.82297 11.3894 6.02547C11.3894 6.22422 11.2244 6.38922 11.0256 6.38922H5.97438C5.77563 6.38922 5.61063 6.22422 5.61063 6.02547C5.61063 5.82297 5.77563 5.66172 5.97438 5.66172H6.40938V5.41797H2.41563C2.02188 5.41797 1.70312 5.73672 1.70312 6.13047V20.8492C1.70312 21.2467 2.02188 21.5617 2.41563 21.5617H14.1156C13.9356 21.0667 13.8269 20.538 13.8269 19.9792C13.8269 18.6517 14.3931 17.4555 15.2969 16.6155V6.13047ZM6.21438 11.4817C6.58187 11.3692 6.99062 11.1667 7.29812 10.9942C7.18187 10.8142 7.08063 10.608 7.00938 10.3792C6.76188 10.1917 6.62687 9.82047 6.58938 9.60297C6.51813 9.23547 6.62313 8.97297 6.79563 8.82297L6.79188 8.70297C6.77313 8.27922 6.91563 7.88547 7.19688 7.59297C7.49688 7.28172 7.90938 7.11297 8.37438 7.11297H8.62938C9.09063 7.11297 9.51062 7.28172 9.80312 7.59297C10.0844 7.88547 10.2231 8.27922 10.2081 8.70297L10.2044 8.82297C10.3806 8.97297 10.4856 9.23547 10.4106 9.60297C10.4106 9.60672 10.4106 9.60672 10.4106 9.60672C10.3694 9.81672 10.2344 10.188 9.99062 10.3792C9.92312 10.608 9.81813 10.8142 9.70188 10.9942C10.0056 11.1667 10.4181 11.3692 10.7894 11.4817C12.0569 11.8642 12.0719 13.518 12.0719 13.5855C12.0719 13.7505 11.9631 13.8892 11.8056 13.938C10.7069 14.2417 9.60062 14.3955 8.49812 14.3955C7.39562 14.3955 6.28938 14.2417 5.19438 13.938C5.03688 13.8892 4.92813 13.7505 4.92813 13.5855C4.92813 13.518 4.94312 11.8642 6.21438 11.4817ZM11.5244 16.0155C11.5244 16.2142 11.3594 16.3792 11.1606 16.3792H5.83938C5.64062 16.3792 5.47563 16.2142 5.47563 16.0155C5.47563 15.8167 5.64062 15.6517 5.83938 15.6517H11.1606C11.3594 15.6517 11.5244 15.8167 11.5244 16.0155ZM12.6119 20.133H4.38812C4.18562 20.133 4.02437 19.9717 4.02437 19.7692C4.02437 19.5705 4.18562 19.4055 4.38812 19.4055H12.6081C12.8106 19.4055 12.9719 19.5705 12.9719 19.7692C12.9756 19.9717 12.8144 20.133 12.6119 20.133ZM12.6119 18.258H4.38812C4.18562 18.258 4.02437 18.093 4.02437 17.8942C4.02437 17.6955 4.18562 17.5305 4.38812 17.5305H12.6081C12.8106 17.5305 12.9719 17.6955 12.9719 17.8942C12.9756 18.093 12.8144 18.258 12.6119 18.258Z'
                        fill='#DFE1ED'
                      />
                      <path
                        d='M20.8465 18.6931L17.7677 21.7719C17.6965 21.8431 17.6027 21.8806 17.509 21.8806C17.4152 21.8806 17.3252 21.8431 17.2502 21.7719L16.0015 20.5231C15.859 20.3806 15.859 20.1519 16.0015 20.0094C16.144 19.8669 16.3727 19.8669 16.5152 20.0094L17.509 20.9994L20.3327 18.1756C20.4752 18.0331 20.704 18.0331 20.8465 18.1756C20.989 18.3181 20.989 18.5506 20.8465 18.6931ZM10.789 11.4781C10.4177 11.3656 10.009 11.1631 9.70148 10.9906C9.81773 10.8106 9.91898 10.6044 9.99023 10.3756C10.2377 10.1881 10.3727 9.81687 10.4102 9.60312C10.4102 9.60312 10.4102 9.60312 10.4102 9.59937C10.4815 9.23187 10.3765 8.96937 10.204 8.81937L10.2077 8.69937C10.2265 8.27562 10.084 7.88188 9.80273 7.58938C9.50648 7.27813 9.09023 7.10938 8.62898 7.10938H8.37398C7.90898 7.10938 7.49273 7.27813 7.19648 7.58938C6.91523 7.88188 6.77648 8.27187 6.79148 8.69937L6.79523 8.81937C6.61898 8.96937 6.51398 9.23187 6.58898 9.59937C6.63023 9.81312 6.76523 10.1844 7.00898 10.3756C7.07648 10.6044 7.18148 10.8106 7.29773 10.9906C6.99023 11.1631 6.58148 11.3656 6.21398 11.4781C4.94273 11.8606 4.92773 13.5144 4.92773 13.5819C4.92773 13.7469 5.03648 13.8856 5.19398 13.9344C6.29273 14.2381 7.39898 14.3919 8.49773 14.3919C9.60398 14.3919 10.7102 14.2381 11.8052 13.9344C11.9627 13.8856 12.0715 13.7469 12.0715 13.5819C12.0715 13.5144 12.0602 11.8606 10.789 11.4781ZM7.29398 9.38562C7.44023 9.32937 7.53773 9.19063 7.53398 9.02938L7.51898 8.67687C7.50773 8.44813 7.57898 8.24937 7.72148 8.09937C7.87898 7.93062 8.10773 7.84437 8.37398 7.84437H8.62898C8.89148 7.84437 9.12023 7.93437 9.27773 8.09937C9.42398 8.24937 9.49148 8.44813 9.48023 8.67687L9.46898 9.02938C9.46148 9.18688 9.56273 9.32562 9.70523 9.38187C9.70523 9.40062 9.70523 9.43062 9.69773 9.46438C9.66773 9.61438 9.59648 9.74562 9.55523 9.79062C9.44273 9.83187 9.35648 9.92562 9.32648 10.0456C9.18773 10.6081 8.84648 11.0056 8.49773 11.0056C8.15273 11.0056 7.81148 10.6081 7.67273 10.0456C7.64273 9.92562 7.55648 9.83187 7.44398 9.79062C7.40273 9.74187 7.33148 9.61063 7.30148 9.46438C7.29773 9.43062 7.29398 9.40062 7.29398 9.38562ZM5.68148 13.3119C5.73398 12.9481 5.89898 12.3294 6.42398 12.1719C6.92648 12.0181 7.45898 11.7406 7.82648 11.5269C8.03273 11.6581 8.26148 11.7294 8.49773 11.7294C8.73773 11.7294 8.96648 11.6581 9.17273 11.5269C9.53648 11.7369 10.0727 12.0181 10.5752 12.1719C11.1002 12.3294 11.2652 12.9481 11.3177 13.3119C9.44648 13.7881 7.55273 13.7881 5.68148 13.3119Z'
                        fill='#181F51'
                      />
                      <path
                        d='M9.70422 9.38125C9.70422 9.4 9.70422 9.43 9.69672 9.46375C9.66672 9.61375 9.59547 9.745 9.55422 9.79C9.44172 9.83125 9.35547 9.925 9.32547 10.045C9.18672 10.6075 8.84547 11.005 8.49672 11.005C8.15172 11.005 7.81047 10.6075 7.67172 10.045C7.64172 9.925 7.55547 9.83125 7.44297 9.79C7.40172 9.74125 7.33047 9.61 7.30047 9.46375C7.29672 9.43 7.29297 9.40375 7.29297 9.385C7.43922 9.32875 7.53672 9.19 7.53297 9.02875L7.51797 8.67625C7.50672 8.4475 7.57797 8.24875 7.72047 8.09875C7.87797 7.93 8.10672 7.84375 8.37297 7.84375H8.62797C8.89047 7.84375 9.11922 7.93375 9.27672 8.09875C9.42297 8.24875 9.49047 8.4475 9.47922 8.67625L9.46797 9.02875C9.46047 9.1825 9.56172 9.32125 9.70422 9.38125Z'
                        fill='#FFE4AA'
                      />
                      <path
                        d='M10.7048 13.3123C9.88359 13.5186 9.06234 13.6348 8.23734 13.6611C8.20734 13.6648 8.17734 13.6648 8.15109 13.6611C7.32609 13.6348 6.50484 13.5186 5.68359 13.3123C5.73609 12.9486 5.90109 12.3298 6.42609 12.1723C6.81234 12.0561 7.21734 11.8611 7.55484 11.6811C7.65234 11.6286 7.74609 11.5761 7.83234 11.5273C7.94859 11.5986 8.07234 11.6548 8.19984 11.6886C8.29734 11.7186 8.40234 11.7336 8.50359 11.7336C8.61984 11.7336 8.73234 11.7148 8.83734 11.6811C9.16734 11.8611 9.57984 12.0561 9.96609 12.1723C10.4836 12.3298 10.6486 12.9486 10.7048 13.3123Z'
                        fill='#F57E20'
                      />
                      <path
                        d='M8.23438 13.6611C9.05938 13.6348 9.88063 13.5186 10.7019 13.3123C10.6456 12.9486 10.4806 12.3298 9.95563 12.1723C9.56938 12.0561 9.16063 11.8648 8.82688 11.6811C8.94688 11.6473 9.05938 11.5986 9.16813 11.5273C9.53188 11.7373 10.0681 12.0186 10.5706 12.1723C11.0956 12.3298 11.2606 12.9486 11.3131 13.3123C10.2931 13.5748 9.26562 13.6911 8.23438 13.6611Z'
                        fill='#AC5816'
                      />
                      <path
                        d='M11.5234 16.0125C11.5234 16.2113 11.3584 16.3763 11.1597 16.3763H5.83844C5.63969 16.3763 5.47469 16.2113 5.47469 16.0125C5.47469 15.8138 5.63969 15.6488 5.83844 15.6488H11.1597C11.3584 15.6488 11.5234 15.8138 11.5234 16.0125ZM12.9747 17.8913C12.9747 18.09 12.8134 18.255 12.6109 18.255H4.38719C4.18469 18.255 4.02344 18.09 4.02344 17.8913C4.02344 17.6925 4.18469 17.5275 4.38719 17.5275H12.6072C12.8134 17.5275 12.9747 17.6888 12.9747 17.8913ZM12.9747 19.7663C12.9747 19.9688 12.8134 20.13 12.6109 20.13H4.38719C4.18469 20.13 4.02344 19.9688 4.02344 19.7663C4.02344 19.5675 4.18469 19.4025 4.38719 19.4025H12.6072C12.8134 19.4025 12.9747 19.5675 12.9747 19.7663ZM8.50094 1.6875C7.90469 1.6875 7.41719 2.175 7.41719 2.7675C7.41719 3.36375 7.90469 3.85125 8.50094 3.85125C9.09344 3.85125 9.58094 3.36375 9.58094 2.7675C9.58094 2.175 9.09344 1.6875 8.50094 1.6875ZM8.50094 3.12375C8.30594 3.12375 8.14469 2.9625 8.14469 2.7675C8.14469 2.57625 8.30594 2.415 8.50094 2.415C8.69219 2.415 8.85344 2.57625 8.85344 2.7675C8.85344 2.9625 8.69594 3.12375 8.50094 3.12375Z'
                        fill='#181F51'
                      />
                    </g>
                    <defs>
                      <clipPath id='clip0_3132_19914'>
                        <rect width='24' height='24' fill='white' transform='translate(0 0.578125)' />
                      </clipPath>
                    </defs>
                  </svg>

                  <span>RBI Approved Lenders</span>
                </li>
                <li>
                  <svg width='24' height='25' viewBox='0 0 24 25' fill='none' xmlns='http://www.w3.org/2000/svg'>
                    <g clip-path='url(#clip0_3132_19924)'>
                      <path
                        d='M16.4508 8.37578C15.3508 9.57578 13.7508 10.3258 12.0008 10.3258C10.2508 10.3258 8.65078 9.57578 7.55078 8.37578C8.65078 7.17578 10.2508 6.42578 12.0008 6.42578C13.7508 6.42578 15.3508 7.17578 16.4508 8.37578ZM16.2508 16.8758C15.1508 17.9758 13.6508 18.6258 12.0008 18.6258C10.3508 18.6258 8.85078 17.9758 7.75078 16.8758C8.85078 15.7758 10.3508 15.1258 12.0008 15.1258C13.6508 15.1258 15.1508 15.8258 16.2508 16.8758Z'
                        fill='#A7ACCE'
                      />
                      <path
                        d='M12.0004 18.6258C15.3693 18.6258 18.1004 15.8947 18.1004 12.5258C18.1004 9.15684 15.3693 6.42578 12.0004 6.42578C8.63145 6.42578 5.90039 9.15684 5.90039 12.5258C5.90039 15.8947 8.63145 18.6258 12.0004 18.6258Z'
                        stroke='#181F51'
                        stroke-width='0.857143'
                        stroke-miterlimit='10'
                      />
                      <path
                        d='M11.9996 18.6258C13.8774 18.6258 15.3996 15.8947 15.3996 12.5258C15.3996 9.15684 13.8774 6.42578 11.9996 6.42578C10.1218 6.42578 8.59961 9.15684 8.59961 12.5258C8.59961 15.8947 10.1218 18.6258 11.9996 18.6258Z'
                        stroke='#181F51'
                        stroke-width='0.857143'
                        stroke-miterlimit='10'
                      />
                      <path
                        d='M12 6.42812V18.5281M18.05 12.4781H5.95M16 2.32812H13.4V4.72813M8 2.32812H10.6V4.72813M8 22.8281H10.6V20.4281M16 22.8281H13.4V20.4281M22.25 16.5781V13.9781H19.85M22.25 8.57812V11.1781H19.85M1.75 8.57812V11.1781H4.15M1.75 16.5781V13.9781H4.15'
                        stroke='#181F51'
                        stroke-width='0.857143'
                        stroke-miterlimit='10'
                      />
                      <path
                        d='M17.25 3.57812C17.9404 3.57812 18.5 3.01848 18.5 2.32812C18.5 1.63777 17.9404 1.07812 17.25 1.07812C16.5596 1.07812 16 1.63777 16 2.32812C16 3.01848 16.5596 3.57812 17.25 3.57812Z'
                        stroke='#F57E20'
                        stroke-width='0.857143'
                        stroke-miterlimit='10'
                      />
                      <path
                        d='M6.75 3.57812C7.44036 3.57812 8 3.01848 8 2.32812C8 1.63777 7.44036 1.07812 6.75 1.07812C6.05964 1.07812 5.5 1.63777 5.5 2.32812C5.5 3.01848 6.05964 3.57812 6.75 3.57812Z'
                        stroke='#4BA264'
                        stroke-width='0.857143'
                        stroke-miterlimit='10'
                      />
                      <path
                        d='M6.75 24.0781C7.44036 24.0781 8 23.5185 8 22.8281C8 22.1378 7.44036 21.5781 6.75 21.5781C6.05964 21.5781 5.5 22.1378 5.5 22.8281C5.5 23.5185 6.05964 24.0781 6.75 24.0781Z'
                        stroke='#F57E20'
                        stroke-width='0.857143'
                        stroke-miterlimit='10'
                      />
                      <path
                        d='M17.25 24.0781C17.9404 24.0781 18.5 23.5185 18.5 22.8281C18.5 22.1378 17.9404 21.5781 17.25 21.5781C16.5596 21.5781 16 22.1378 16 22.8281C16 23.5185 16.5596 24.0781 17.25 24.0781Z'
                        stroke='#4BA264'
                        stroke-width='0.857143'
                        stroke-miterlimit='10'
                      />
                      <path
                        d='M22.25 19.0781C22.9404 19.0781 23.5 18.5185 23.5 17.8281C23.5 17.1378 22.9404 16.5781 22.25 16.5781C21.5596 16.5781 21 17.1378 21 17.8281C21 18.5185 21.5596 19.0781 22.25 19.0781Z'
                        stroke='#283487'
                        stroke-width='0.857143'
                        stroke-miterlimit='10'
                      />
                      <path
                        d='M22.25 8.57812C22.9404 8.57812 23.5 8.01848 23.5 7.32812C23.5 6.63777 22.9404 6.07812 22.25 6.07812C21.5596 6.07812 21 6.63777 21 7.32812C21 8.01848 21.5596 8.57812 22.25 8.57812Z'
                        stroke='#4BA264'
                        stroke-width='0.857143'
                        stroke-miterlimit='10'
                      />
                      <path
                        d='M1.75 8.57812C2.44036 8.57812 3 8.01848 3 7.32812C3 6.63777 2.44036 6.07812 1.75 6.07812C1.05964 6.07812 0.5 6.63777 0.5 7.32812C0.5 8.01848 1.05964 8.57812 1.75 8.57812Z'
                        stroke='#283487'
                        stroke-width='0.857143'
                        stroke-miterlimit='10'
                      />
                      <path
                        d='M1.75 19.0781C2.44036 19.0781 3 18.5185 3 17.8281C3 17.1378 2.44036 16.5781 1.75 16.5781C1.05964 16.5781 0.5 17.1378 0.5 17.8281C0.5 18.5185 1.05964 19.0781 1.75 19.0781Z'
                        stroke='#4BA264'
                        stroke-width='0.857143'
                        stroke-miterlimit='10'
                      />
                    </g>
                    <defs>
                      <clipPath id='clip0_3132_19924'>
                        <rect width='24' height='24' fill='white' transform='translate(0 0.578125)' />
                      </clipPath>
                    </defs>
                  </svg>

                  <span>Easy Digital Process</span>
                </li>
              </ul>
            </Col>
            <Col xs={5}>
              <CustomImage src={personalLoanBenefits} alt='Key Benefits of Kashti’s Personal Loan ' />
            </Col>
          </Row>
        </Container>
      </section>

      {/* Why Kashti */}
      <section className={styles.whyKashtiSection} id='key_features'>
        <Container>
          <Row>
            <Col lg={12}>
              <SectionTitle>Trustworthy Loans with Kashti</SectionTitle>
            </Col>
          </Row>
          <Row>
            <Col xs={12} md={6} lg={4}>
              <div className={styles.whyKashtiBox}>
                <CustomImage src={instantLoan} alt='Safe & secure' />
                <h3>Instant Loan Approval</h3>
                <Text>Instant access to funds without hassles and delays.</Text>
              </div>
            </Col>
            <Col xs={12} md={6} lg={4}>
              <div className={styles.whyKashtiBox}>
                <CustomImage src={interestRates} alt='Safe & secure' />
                <h3>Attractive Interest Rates</h3>
                <Text>Loans with the most value and the best interest rates.</Text>
              </div>
            </Col>
            <Col xs={12} md={6} lg={4}>
              <div className={styles.whyKashtiBox}>
                <CustomImage src={flexibleTenures} alt='Diverse Options' />
                <h3>Flexible Tenures</h3>
                <Text>Choice of tenure options for convenient repayment.</Text>
              </div>
            </Col>
            <Col xs={12} md={6} lg={4}>
              <div className={styles.whyKashtiBox}>
                <CustomImage src={approvedLenders} alt='Quick Disbursal' />
                <h3>RBI Approved Lenders</h3>
                <Text>Complete peace of mind with 100% approved lenders.</Text>
              </div>
            </Col>
            <Col xs={12} md={6} lg={4}>
              <div className={styles.whyKashtiBox}>
                <CustomImage src={digitalProcess} alt='Anytime Support' />
                <h3>Easy Digital Process</h3>
                <Text>Efficient and user-friendly digital processing of loans.</Text>
              </div>
            </Col>
          </Row>
        </Container>
      </section>

      {/* Check out attractive interest rates with our affiliated banks and NBFCs. */}
      <section className={styles.attractiveRatesSection} id='interest_rates'>
        <Container>
          <Row>
            <Col lg={12}>
              <SectionTitle>Attractive Interest Rates, Always</SectionTitle>
              <div className={styles.creditCardBlock}>
                <CreditCardBox themeColor={"#E9F4EC"} />
                <CreditCardBox themeColor={"#FEF2E9"} />
                <CreditCardBox themeColor={"#EAEBF3"} />
                <CreditCardBox themeColor={"#EAEBF3"} />
                <CreditCardBox themeColor={"#E9F4EC"} />
                <CreditCardBox themeColor={"#FEF2E9"} />
                <CreditCardBox themeColor={"#FEF2E9"} />
                <CreditCardBox themeColor={"#EAEBF3"} />
                <CreditCardBox themeColor={"#E9F4EC"} />
              </div>
              <div className={styles.viewAllCreditCard}>
                <Link href='#' className='secondaryBtn'>
                  View All
                </Link>
              </div>
            </Col>
          </Row>
        </Container>
      </section>

      {/* EMI Calculator */}
      <EmiCalculator color={"#1B6430"} id='emi_calculator' />

      {/* Eligibility Criteria for Personal Loan */}
      <section className={styles.eligibilityCriteriaSection} id='eligibility'>
        <Container>
          <Row>
            <Col lg={12}>
              <SectionTitle>Eligibility Criteria for Personal Loan</SectionTitle>
              <Text>Loan eligibility can vary based on the lender's credit risk policies. Here are some criteria that can influence approval:</Text>
              <div className={styles.eligibilitySteps}>
                <div className={styles.eligibilityBox}>
                  <Text>
                    <span>Age</span>
                    <span>18-60 Years</span>
                  </Text>
                  <CustomImage src={age} alt='Age' />
                </div>
                <div className={styles.eligibilityBox}>
                  <Text>
                    <span>Salaried</span>
                    <span>₹15,000/Month</span>
                  </Text>
                  <CustomImage src={salaried} alt='Salaried' />
                </div>
                <div className={styles.eligibilityBox}>
                  <Text>
                    <span>Self-employed</span>
                    <span>&gt;₹5 Lakh p.a.</span>
                  </Text>
                  <CustomImage src={selfEmployed} alt='Self Employed' />
                </div>
                <div className={styles.eligibilityBox}>
                  <Text>
                    <span>Credit Score</span>
                    <span>&gt;750</span>
                  </Text>
                  <CustomImage src={creditScore} alt='Credit score' />
                </div>
                <div className={styles.eligibilityBox}>
                  <Text>
                    <span>Work Experience</span>
                    <span>&gt;1 Year</span>
                  </Text>
                  <CustomImage src={workExperience} alt='Work Experience' />
                </div>
                <div className={styles.eligibilityBox}>
                  <Text>
                    <span>Business Continuity</span>
                    <span>&gt;3 Years</span>
                  </Text>
                  <CustomImage src={creditScore} alt='Business Continuity' />
                </div>
              </div>
              <Text className={styles.notes}>
                *Employment type includes Salaried employees working with reputed organisations, MNCs, Private and Public Limited Companies, Govt.
                organisations, PSUs, and large enterprises
              </Text>
            </Col>
          </Row>
        </Container>
      </section>

      {/* Secure an Instant Personal Loan with these 5 Easy Steps  */}
      <section className={styles.instantPersonalLoanSection}>
        <Container>
          <Row>
            <SectionTitle>5 Steps to Instant Loans</SectionTitle>
            <Text>Simple and Effortless with Kashti.</Text>
          </Row>
          <Row>
            <Col xs={12} md={12} lg={6}>
              <ul className={styles.instantPersonalLoanStepsList}>
                <li>
                  <h4>1</h4>
                  <div>
                    <h3>Check Your Eligibility</h3>
                    <p>Enter your mobile number and instantly check your eligibility.</p>
                  </div>
                </li>
                <li>
                  <h4>2</h4>
                  <div>
                    <h3>Choose Loan Details</h3>
                    <p>Provide a few basic details and verify it using an OTP.</p>
                  </div>
                </li>
                <li>
                  <h4>3</h4>
                  <div>
                    <h3>Compare Options</h3>
                    <p>Compare bank offers to find the best option for you.</p>
                  </div>
                </li>
                <li>
                  <h4>4</h4>
                  <div>
                    <h3>Verify Your Details</h3>
                    <p>Complete your verification by providing a few necessary details.</p>
                  </div>
                </li>
                <li>
                  <h4>5</h4>
                  <div>
                    <h3>Instant Money Transfer</h3>
                    <p>Enter your bank details, set up auto-debit and get the loan amount transferred within minutes.</p>
                  </div>
                </li>
              </ul>
            </Col>
            <Col xs={12} md={6} lg={6}>
              <CustomImage src={instantPersonalLoan} alt='Secure an Instant Personal Loan with these 5 Easy Steps' />
            </Col>
          </Row>
        </Container>
      </section>

      {/* Frequently Asked Questions  */}
      <FaqSection activeColor={"#F2F8F4"} id='faqs' />

      {/* Quick Links */}
      <section className={styles.quickLinksSection}>
        <Container>
          <Row>
            <Col lg={12}>
              <SectionTitle>Quick Links</SectionTitle>
            </Col>
          </Row>
          <Row>
            <Col xs={6} md={5} lg={3}>
              <div className={styles.quickLinkBox}>
                <BusinessLoanIcon />
                <h4>Business Loan</h4>
                <p>Helps your business grow better by approving loan with low interest rates.</p>
              </div>
            </Col>
            <Col xs={6} md={5} lg={3}>
              <div className={styles.quickLinkBox}>
                <GoldLoanIcon />
                <h4>Gold Loan</h4>
                <p>We help you to choose the Best rates for your Gold. </p>
              </div>
            </Col>
          </Row>
        </Container>
      </section>
    </main>
  );
};

export default Page;
